...
const Navbar = () => {
  ...
  const navLinks = (
    <>
      ...
      <NavLink to="/courts" className={navLinkClass} onClick={() => setMobileMenuOpen(false)}>Courts</NavLink>
      <NavLink to="/membership" className={navLinkClass} onClick={() => setMobileMenuOpen(false)}>Membership</NavLink>
      <NavLink to="/terms-and-policies" className={navLinkClass} onClick={() => setMobileMenuOpen(false)}>Terms & Policies</NavLink>
      {!user && <NavLink to="/login" className={navLinkClass} onClick={() => setMobileMenuOpen(false)}>Login</NavLink>}
      <ThemeToggle />
    </>
  );
                  ...
                  <AnimatePresence>
                    {dropdownOpen && (
                      <motion.div
                        ...
                        className="absolute right-0 z-50 w-48 p-4 mt-2 space-y-2 rounded-md shadow-lg bg-white/90 backdrop-blur-md dark:bg-black/80"
                      >
                        <p className="text-sm font-semibold dark:text-white">{user.displayName || user.email}</p>
                        <Link
                          to="/dashboard"
                          onClick={() => { toggleDropdown(); setMobileMenuOpen(false); }}
      ...
      <AnimatePresence>
        {mobileMenuOpen && (
          <motion.aside
            ...
            className="fixed inset-y-0 left-0 z-50 w-64 p-6 overflow-y-auto shadow-lg bg-white/90 backdrop-blur-md dark:bg-black/80 md:hidden"
          >
