    ...
    <div className="px-5 mt-32 mb-20">
      <div className="grid grid-cols-1 gap-6 mx-auto sm:grid-cols-2 lg:grid-cols-4 max-w-7xl">
        {stats.map((stat, index) => (
          ...
          >
            <div className="flex justify-center text-[var(--color-primary)] dark:text-[var(--color-secondary)] mb-3">
              {stat.icon}
            </div>
            <h3 className="text-3xl font-bold text-[var(--color-text-primary)] dark:text-white mb-1">
              {stat.value}
            </h3>
            <p className="text-[var(--color-text-secondary)] dark:text-gray-300 font-medium">
              {stat.label}
            </p>
          </motion.div>
        ))}
